...
<div class="barangay-admins py-2 px-4">
    <div id="title">
        <h1 class="text-lg sm:text-2xl font-bold text-blue-600">ADMINISTRATORS:</h1>
    </div>
    <div class="my-6">
...
<!-- Filter Form using Select2 -->
    <div class="mb-4"> 
      <form action="{{ route('lgu.admins') }}" method="GET" id="filterForm">
        <p class="mb-2 text-gray-600 text-[13px] lg:text-[15px] font-semibold">Filter by Barangay:</p>
    ...
    <div class="mb-2 flex justify-end">
        ...
        <button class="relative text-left px-4 py-2 text-red-600 text-[7px] lg:text-[15px] hover:text-red-800 rounded-lg group">
            <span class="font-bold">Caution <i class="fa-solid fa-triangle-exclamation"></i></span>
            <div class="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 hidden group-hover:block bg-white text-red-600 text-[7px] lg:text-[15px] p-4 rounded shadow-lg w-64">
         ...
         <!-- Create -->
        <a href="{{ route('lgu.create-barangay') }}" class="py-2 px-4 text-[7px] lg:text-[15px] bg-blue-600 text-white font-bold rounded hover:bg-blue-500"><i class="fa-solid fa-plus"></i> Add Barangay Admin</a>
    </div>
    ...
    <div class="max-h-[40vh] overflow-y-auto">
        <table class="min-w-full table-auto bg-white border border-gray-300 rounded-lg shadow-md">
            <thead class="bg-gray-600 text-white">
                <tr>
                    <th class="py-3 px-4 lg:px-6 bg-gray-600 text-white font-bold uppercase text-[7px] lg:text-[13px] text-left">Name</th>
                    <th class="py-3 px-4 lg:px-6 bg-gray-600 text-white font-bold uppercase text-[7px] lg:text-[13px] text-left">Email</th>
                    <th class="py-3 px-4 lg:px-6 bg-gray-600 text-white font-bold uppercase text-[7px] lg:text-[13px] text-left">Barangay</th>
                    <th class="py-3 px-4 lg:px-6 bg-gray-600 text-white font-bold uppercase text-[7px] lg:text-[13px] text-left">Update</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr class="hover:bg-gray-300 transition duration-300 ease-in-out">
                    <td class="py-2 px-4 font-semibold border-b text-[7px] lg:text-[15px] border-gray-200">{{ $admin->name }}</td>
                    <td class="py-2 px-4 font-semibold border-b text-[7px] lg:text-[15px] text-blue-600 border-gray-200">{{ $admin->email }}</td>
                    <td class="py-2 px-4 font-semibold border-b text-[7px] lg:text-[15px] border-gray-200">{{ $admin->barangay ? $admin->barangay->barangay_name : 'N/A' }}</td>
